...
def collect_cassandra_store_settings():
    ...
    db_type = click.prompt(
        'Regular [C]assandra or [A]stra DB?',type=click.Choice(['C','A']),
        show_choices=False,
        default='C',
    )
    is_astra = db_type == 'A'
    ...
    if is_astra:
        ...
        c_hosts = None
        c_port = None
        c_username = click.prompt(
            'Enter the Client ID from your Astra DB token'
        )
        ...
        hosts_string = click.prompt(
            (
                'Enter the seed hosts of your cluster '
...
def apply_cassandra_store_settings(config_file, settings):
    ...
    if settings['c_hosts']:
        replace_str_in_file(
            ...
            os.linesep.join(
                f'        - {c_host}'
                for c_host in settings['c_hosts']
            ),
        )
    else:
...
def write_setting_or_remove(file_path, setting_value, setting_name, setting_placeholder_value):
    """
